**diffstat命令**用来显示[diff](#/diff "diff命令")命令输出信息的柱状图，用以显示diff命令比较两个文件的不同统计信息。用户也可以直接使用`|`将diff命令所输出的结果直接送给diffstat命令进行统计结果的显示。使用该命令时，若所比较的文件或者子目录不在当前目录下，则应该使用其完整路径。
...
对于查看文件中的内容，用户可以通过[cat](#/cat "cat命令")命令进行查看即可，具体操作如下：
...
```
testfile | 2 +-             #统计信息输出显示
1 [file](#/file "file命令") changed, 1 insertion(+), 1 deletion(-)
```