**depmod命令**可产生模块依赖的映射文件，在构建嵌入式系统时，需要由这个命令来生成相应的文件，由[modprobe](#/modprobe "modprobe命令")使用。
...
*   `/home/windsome/EMMA3PF-KernelSource-20080626/install_pos`是我`[make](#/make "make命令") mod_install`后，所有模块的存放路径。
...
```
genkernel.sh ([at](#/at "at命令") linux-2.6.18_pro500)
#######
...
在linux桌面系统中，当你编译了新的驱动，为了能够用`modprobe ***`加载模块, 你需要先将模块拷贝到`/lib/modules /2.6.31-20-generic`目录下，然后运行`[sudo](#/sudo "sudo命令") depmod -a`将模块信息写入modules.dep、modules.dep.bin、modules.[alias](#/alias "alias命令").bin、modules.alias和modules.pcimap文件中。
