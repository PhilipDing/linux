...
eject允许可移动介质（典型是[cd](https://philipding.github.io/linux-command/cd "cd命令")-ROM、软盘、磁带、或者JAZ以及[zip](https://philipding.github.io/linux-command/zip "zip命令")磁盘）在软件控制下弹出。该命令也可以控制一些多盘片CD-ROM控制器，控制一些设备支持的自动弹出功能，以及控制一些CD-ROM驱动器磁盘托盘的关闭。与name相应的设备将被弹出，name可以为设备文件或者其挂载点，也可以为完整路径或者省略前面的/dev或者/mnt设备文件名。如果没有指定name，缺省使用cdrom。
