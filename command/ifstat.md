**ifstat命令**就像[iostat](#/iostat "iostat命令")/[vmstat](#/vmstat "vmstat命令")描述其它的系统状况一样，是一个统计网络接口活动状态的工具。ifstat工具系统中并不默认安装，需要自己下载源码包，重新编译安装，使用过程相对比较简单。

### 下载  

```
http://gael.roualland.[free](#/free "free命令").fr/ifstat/  （官网）
...
注释：执行`[which](#/which "which命令") ifstat`输出`/usr/local/bin/ifstat`
