**lp命令**用于打印文件，或者修改排队的打印任务。与[lpr](https://philipding.github.io/linux-command/lpr "lpr命令")命令类似，lp命令既支持文件输入也支持标准输入。它与lpr的不同之处在于它有一个不同（稍微复杂点）的参数选项设置。
...
-d：指定接收打印任务的目标打印机；
-i：指定一个存在的打印任务号；
-m：打印完成时发送E-[mail](https://philipding.github.io/linux-command/mail "mail命令")；
-n：指定打印的份数；
-t：指定打印任务的名称；
-H：指定打印任务开始的时间；
-P：指定需要打印的页码。
